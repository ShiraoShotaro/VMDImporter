...
public class CameraControl : MonoBehaviour
{
	...
	public float mouseTranslateSensitivity = 0.01f;
	public float mouseRotateSensitivity = 0.1f;
	...
	void mouseDragEvent(Vector3 mousePos)
	{
		...
		else if (Input.GetMouseButton((int)MouseButtonDown.MBD_MIDDLE))
		{
			...
			Vector3 diff_yinv = diff;
			diff_yinv.y *= -1;
			this.cameraTranslate(diff_yinv * mouseTranslateSensitivity);

		}
		else if (Input.GetMouseButton((int)MouseButtonDown.MBD_RIGHT))
		{
			// マウス右ボタンをドラッグした場合(カメラの回転)
			this.cameraRotate(new Vector3(diff.y, diff.x, 0.0f) * mouseRotateSensitivity);
		}
